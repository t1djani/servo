...
2. **Convene the MINIMAL relevant set — usually 2-3, not every plausibly-related domain.** From the manifest's `experts`, pick only the ones the decision genuinely turns on (a data fork wakes the data expert; it does not need the bot expert). Convening six experts on a three-expert problem is the waste this step must avoid. Spawn each as a subagent that **reads its own specific `sources`** and argues from its domain — but hand it the **context brief** (from `gather-context`) so it does not re-read the whole project; it only goes deeper into its own slice. Each expert is a real, separate perspective — not you role-playing all of them. These reader-experts can run on a **cheaper, faster model**; reserve the strong model for the synthesis in step 5.
