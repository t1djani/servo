...
2. **Infer the experts (domains).** From the code layout and docs, infer the project's domains (frontend, data, auth, …). For each, a seat is a **role plus pointers to where its knowledge lives** — its depth comes from the sources it points at, read when convened, not from a baked-in description. A scanned "design expert" that points at the real tokens file is deep; a generic one is not.
